...
class User implements UserInterface
{
     ...
     * @Assert\NotBlank(message="Please enter a valid passwod")
    ...
    /**
     * @ORM\Column(type="string", length=45)
     * @Assert\NotBlank (message="Valid last name id required.")
     */
    private $last_name;
    ...
    public function addDislikeVideo(Video $dislikeVideo): self
    {
        if (!$this->dislikeVideos->contains($dislikeVideo)) {
            ...
            $dislikeVideo->addUserThatDontLike($this);
        }
    ...
    {
        if ($this->dislikeVideos->contains($dislikeVideo)) {
            $this->dislikeVideos->removeElement($dislikeVideo);
            $dislikeVideo->removeUserThatDontLike($this);
        }
